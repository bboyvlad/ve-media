<?php
	...
	$an_rutagalery="ruta/de/la/galeria/";
		...
		if ($ex>0) {
			...
			foreach($rs as $value){
 			...
 			if ($a_passwd==$a_clave) {
				...
				$an_status='0';
				$publica=true;
				echo '{"mensaje":"El Usuario ya ha Publicado antes"}';

 			}else{
 				//Tendra que Recuperar la Contraseña
 				echo '{"mensaje":"Tendra que Recuperar la Contraseña"}';
 				$publica=false;
			...
			//echo "'mensaje':'Registro Primero el Anunciante'";
			$publica=true;	
		}
		...
		if ($publica==true){
			...
			echo '{"mensaje":"Registro el anuncio y Registro Primero el Anunciante"}';
